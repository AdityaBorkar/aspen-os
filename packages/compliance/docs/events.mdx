...
The Compliance module defines 23 domain events covering document lifecycle, obligation management, and scheduled jobs.
...
| Event                           | Payload                                                                     | Trigger                             |
| ------------------------------- | --------------------------------------------------------------------------- | ----------------------------------- |
| `compliance:document_expiring`  | `{ daysUntilExpiry, documentId, sourceEntityId, sourceModule, recipient? }` | Expiry reminder threshold reached   |
| `compliance:document_due`       | `{ daysUntilDue, documentId, sourceEntityId, sourceModule, recipient? }`    | Due date reminder threshold reached |
...
await p.pubsub.subscribe("compliance:document_verified", async (message) => {
  const { documentId, verifiedBy, category } = message.data;
  // Send notification, update dashboard, etc.
});

// React to expiry reminders
await p.pubsub.subscribe("compliance:document_expiring", async (message) => {
  const { documentId, daysUntilExpiry } = message.data;
  // Send email notification
});

// Listen to weekly summary
await p.pubsub.subscribe("compliance:weekly_summary", async (message) => {

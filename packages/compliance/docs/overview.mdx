...
2. **Event bridge** — `registerEventBridgeSubscriptions({ audit, db, pubsub })` subscribes 8 topics (6 logical peers; `branch:created` aliases) validating with Valibot and deduping by `{sourceModule, sourceEntityId, documentType}`.
...
<Callout type="warn">
  `CRON_SCHEDULES` and `SCHEDULED_JOBS` (`daily-expiry-scan`, `daily-status-transition`,
  `daily-escalation`, `weekly-summary`, plus `obligation-generate` at `0 6 * * *`) are defined in
  `src/utils/constants.ts` but **never passed to `pubsub.schedule()`**. The obligation generator
  subscribes to its topic — trigger it externally or via `generatePendingDocuments()` until a
  schedule is registered. Reminder jobs are deprecated; see `services/reminder-engine.ts` stub.
</Callout>

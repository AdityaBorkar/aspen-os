---
title: Events
description: Domain event map for the Masters module — 31 events across 8 event groups.
icon: IconBell
---

The Masters module defines 31 domain events across 8 event groups. Events are type-level contracts published as plain string topics via PubSub. The `masters:contact_created` event replaces the old `organization:connection_created` for the compliance insurer flow.
...
| `masters:unit_of_measure_activated`   | `{ unitOfMeasureId }`                                | UOM activated   |
| `masters:unit_of_measure_deactivated` | `{ unitOfMeasureId }`                                | UOM deactivated |

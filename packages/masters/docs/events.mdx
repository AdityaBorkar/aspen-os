---
title: Events
description: Domain event map for the Masters module — 29 events across 7 event groups.
icon: IconBell
---

The Masters module defines 29 domain events across 7 event groups. Events are type-level contracts published as plain string topics via PubSub. The `masters:contact_created` event replaces the old `organization:connection_created` for the compliance insurer flow.

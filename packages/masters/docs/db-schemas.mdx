...
Tenant tables with `master_` and `org_branch` prefixes and no control-plane tables; shared columns `id` (text PK, UUIDv7), `created_at`, `updated_at` (timestamptz) apply to all tables and are not repeated per table.
...
| `master_uom_category_enum`             | `area`, `count`, `data`, `length`, `mass`, `other`, `session`, `temperature`, `time`, `volume`                                                                                  |
...
Indexes: unique `idx_master_uom_code` (`code`), `idx_master_uom_category` (`category`), `idx_master_uom_is_active` (`is_active`), `idx_master_uom_status` (`status`), `idx_master_uom_symbol` (`symbol`).

### `master_uom_alias`

Alias rows pointing at a unit of measure.

| Column   | Type | Description     |
| -------- | ---- | --------------- |
| `alias`  | text | Unique alias    |
| `uom_id` | text | Owning unit row |

Indexes: unique `idx_master_uom_alias_alias` (`alias`), `idx_master_uom_alias_uom_id` (`uom_id`).

### `master_uom_version`

Effective-dated conversion versions for a unit of measure.
...
Indexes: `idx_master_uom_version_uom_id` (`uom_id`), `idx_master_uom_version_effective` (`uom_id`, `effective_from`).

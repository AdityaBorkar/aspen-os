...
The Management Plane declares control-plane tables and empty tenant schemas. Shared columns `id`, `created_at`, `updated_at` apply to all tables and are not repeated per table; `tenant.id` is text sharing better-auth `organization.id`.

...
export const listFolders = Workflow.name("drive.folder.list").handler(async (input, ctx) => {
  ...
  if (validated.search) {
    folderConditions.push(sql`${driveFolder.name} ilike ${`%${validated.search}%`}`);
  }
  ...
  if (validated.search) {
    fileConditions.push(sql`${driveFile.name} ilike ${`%${validated.search}%`}`);
  }

...
The Comms module declares a control-plane `provider` table plus tenant tables (`channel`, `notification`, `message`, `preference`, `template`, `setting`). Code symbols carry the `comms` prefix (`commsChannel`, `commsProvider`, …). Every table uses `id: uuidv7("id").primaryKey()` and `timestamptz` timestamps (`message` tracks lifecycle timestamps instead of `updated_at`). `channel.entityType` reuses `MASTER_ENTITY_TYPE` values (constants-only; no `masters` dependency).
...
Indexes: `idx_provider_kind(kind)`, `idx_provider_active(is_active)`.
...
Indexes: `idx_channel_scope(entity_type, entity_id)`, `idx_channel_type(type)`, `idx_channel_status(status)`, `idx_channel_provider(provider_id)`.
...
Indexes: `idx_notification_recipient(recipient_type, recipient_id)`, `idx_notification_status(status)`, `idx_notification_type(type)`, `idx_notification_created(created_at)`.
...
Indexes: `idx_message_status(status)`, `idx_message_channel(channel_id)`, `idx_message_notification(notification_id)`, `idx_message_provider(provider_message_id)`, `idx_message_created(created_at)`, `idx_message_tenant(tenant_id)`.
...
Indexes: `idx_preference_user(user_id, type, channel_type)`.
...
| `channel_type`              | `channel_type` | `email`, `sms`, `whatsapp`                        |
...
Indexes: `idx_template_channel_name(channel_type, name)`.
...
```mermaid
flowchart LR
  P[provider] -->|provider_id soft FK| C[channel]
  C -->|channel_id soft FK| M[message]
  N[notification] -->|notification_id soft FK| M
  T[template] -->|template_id soft FK| M
```

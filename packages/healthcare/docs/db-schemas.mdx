...
Role membership lives in HR access control (`p.hrCore.access.roles`); no role workflows here beyond the table.
...
| `impression`                | text        |                            |
| `dms_file_id`               | text        | (FK → dms.file, soft)      |
| `version`                   | integer     |                            |
| `authorized_by`             | text        |                            |
| `status`                    | text        |                            |
| `payload`                   | jsonb       |                            |
...
| `encounter_id`                | text               |                              |
| `label`                       | text               |                              |
| `dms_file_id` / `file_type`   | text               | (FK → dms.file, soft)        |
...
Generic grant store; `consents-get` now returns a unified list across this store plus the patient, dental, and caregiver consent stores.
...
Reports run only against the 17-table explorer allowlist.

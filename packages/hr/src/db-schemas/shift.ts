...
export const shiftType = pgTable(
  "shift_type",
  {
    allowCheckOutAfterEnd: integer("allow_check_out_after_end").notNull().default(0),
    allowOvertime: boolean("allow_overtime").notNull().default(false),
    beginCheckInBeforeStart: integer("begin_check_in_before_start").notNull().default(0),
    createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
    determineCheckInBy: text("determine_check_in_by"),
    earlyExitGraceMinutes: integer("early_exit_grace_minutes").notNull().default(0),
    enableAutoAttendance: boolean("enable_auto_attendance").notNull().default(false),
    enableAutoUpdateSync: boolean("enable_auto_update_sync").notNull().default(false),
    endTime: text("end_time").notNull(),
    holidayList: text("holiday_list"),
    id: uuidv7("id").primaryKey(),
    isActive: boolean("is_active").notNull().default(true),
    lateEntryGraceMinutes: integer("late_entry_grace_minutes").notNull().default(0),
    markAttendanceOnHolidays: boolean("mark_attendance_on_holidays").notNull().default(false),
    ...
    workingHoursThresholdForHalfDay: text("working_hours_threshold_for_half_day"),
  },
...
export const shiftScheduleAssignment = pgTable(
  ...
  (table) => [index("idx_shift_schedule_assignment_employee_id").on(table.employeeId)],
);

...
export const getAnnouncementStats = Workflow.name("announcement.stats.get")
  .input(InputSchema)
  .handler(async ({ announcementId }, ctx) => {
    await fetchAnnouncementById(ctx.db, announcementId);

    const recipients = await ctx.db
      .select({ hasUser: announcementRecipient.user_id })
      .from(announcementRecipient)
      .where(eq(announcementRecipient.announcement_id, announcementId));
    ...
    const statusCounts = await ctx.db.execute<{ count: number; status: string }>(sql`
      ...
      FROM notification
      WHERE source_module = 'announcement'

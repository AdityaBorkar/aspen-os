...
The hr-leave module owns leave types, policies, allocations, applications, encashment, compensatory leave, block lists, adjustments, the append-only ledger, and holiday lists/holidays — all tenant-scoped and linked to hr-core employees via soft `employee_id`.
...
flowchart TB
  ...
  PI --> ACL[auth.acl\nleave, config]
  PI --> TS[tenant_schemas\nleave_type, leave_period, leave_policy, leave_allocation, leave_application, leave_encashment, leave_ledger_entry, holiday_list, holiday...]
  PI --> EV[events\nleave]
...
`holiday_list` is a named yearly calendar (`name`, `year`, `weekly_off_days` pattern, soft-delete via `is_active`); `holiday` is a dated entry within a list (`holiday_list_id`, `name`, `date`, `hr_holiday_type`). Served by the `config` workflow group (`p.hrLeave.config.holidayLists` / `p.hrLeave.config.holidays`); `createHoliday` validates the owning list exists.

...
Enums: see [Enums](/docs/hr-leave/enums).
...
Named holiday calendar; `delete` soft-deletes via `is_active`.
...
Single holiday within a list; `delete` hard-deletes.

| Column            | Type              | Description      |
| ----------------- | ----------------- | ---------------- |
| `holiday_list_id` | text (FK → list)  | Owning list      |

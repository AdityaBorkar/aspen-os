...
The DMS module declares tenant tables with the `dms_` prefix. Shared columns `id`, `created_at`, `updated_at` appear on most tables and are omitted per-table below — `dms_label_cache` mirrors `masters` label ids (`text PK`, `updated_at` only). Labels are defined in `masters` (`master_label`); DMS stores only the `dms_entity_label` join plus the `dms_label_cache` projection.
...
Indexes: `idx_dms_file_search(GIN expression)`, `idx_dms_file_folder(folder_id)`, `idx_dms_file_class(class_id)`, `idx_dms_file_status(status)`, `idx_dms_file_owner(owner_id)`, `idx_dms_file_batch(batch_id)`, `idx_dms_file_expiry(expiry_date)`, `idx_dms_file_path(path)`.
...
Indexes: unique `idx_dms_file_version_file_ver(file_id, version)`, `idx_dms_file_version_file(file_id)`.
...
Indexes: `idx_dms_folder_parent(parent_id)`, `idx_dms_folder_owner(owner_id)`, `idx_dms_folder_path(path)`, `idx_dms_folder_trashed(is_trashed)`.
...
Indexes: `idx_dms_class_field_class(class_id)`, unique `idx_dms_class_field_class_name(class_id, name)`.
...
Indexes: unique `idx_dms_entity_label_unique(entity_type, entity_id, label_id)`, `idx_dms_entity_label_entity(entity_type, entity_id)`, `idx_dms_entity_label_label(label_id)`.
...
Indexes: unique `idx_dms_share_entity_grantee(entity_type, entity_id, grantee_type, grantee_id)`, `idx_dms_share_entity(entity_id, entity_type)`, `idx_dms_share_grantee(grantee_id, grantee_type)`.
...
Indexes: `idx_dms_public_link_entity(entity_id, entity_type)`, `idx_dms_public_link_active(is_active)`.
...
Indexes: `idx_dms_legal_hold_file(file_id)`, `idx_dms_legal_hold_released(released_at)`.
...
Indexes: `idx_dms_setting_key(key)`.
...
Indexes: `idx_dms_access_log_entity(entity_id, entity_type)`, `idx_dms_access_log_public_link(public_link_id)`, `idx_dms_access_log_created(created_at)`.

### `dms_label_cache`

Projection of `masters` label taxonomy synced from `masters.label_created / label_updated / label_removed`; DMS never reads `master_label` directly.

| Column       | Type        | Description         |
| ------------ | ----------- | ------------------- |
| `id`         | text (PK)   | Masters label id    |
| `name`       | text        | Label name          |
| `color`      | text        | Label color         |
| `scope_type` | text        | Label scope type    |
| `scope_id`   | text        | Label scope id      |
| `updated_at` | timestamptz | Last sync timestamp |

Indexes: `idx_dms_label_cache_name(name)`, `idx_dms_label_cache_scope(scope_type, scope_id)`.
...
flowchart LR
  ...
  FO --> F
  F & FO --> EL[dms_entity_label] --> LC[dms_label_cache]
  LC -. synced from .-> ML[master_label<br/>masters]
  F & FO --> S[dms_share]

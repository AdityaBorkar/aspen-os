...
`$consumes` lists the contact-share bridge topic plus the label-bridge projection topics — removing a Masters contact revokes every DMS share granted to that contact, and `masters.label_*` events sync the local `dms_label_cache`. The platform does not validate `$consumes`.
...
flowchart TB
  ...
  masters -. label_created/updated/removed<br/>syncs LC .-> LC
...
Shares are grants to `user`/`group`/`contact` over polymorphic `file`/`folder` entities; folder grants inherit down the tree. `contact` grantees hold Masters contact ids — invalidated on `masters.contact_removed`. Labels live in `masters` (`master_label`); `dms_entity_label` links them to files/folders and `dms_label_cache` mirrors label names for search. Legal holds block purge.
...
flowchart LR
  ...
  FO --> F
  F & FO -->|entity_type+entity_id| EL[dms_entity_label] -->|label_id| LC[dms_label_cache]
  LC -. synced from .-> ML[master_label]
...
<Callout type="info">
  DMS is the single document/file surface — no parallel `drive` model. Contacts and label
  definitions stay in `masters`; DMS keeps only grants, joins, and the `dms_label_cache` projection.
</Callout>
...
`$prepareRuntime()` registers `dms.expiry-scan` and `dms.auto-purge` plus the contact-share and label bridges; `$cleanup()` unregisters all four.
...
packages/dms/src/
  ...
  pubsub.ts    # events across 5 groups
  db-schemas/  # tenant tables + 6 enums
  workflows/   # 16 groups — one file per action
  services/    # expiry-scanner, purge-service, contact-share-bridge, label-bridge, paths, storage

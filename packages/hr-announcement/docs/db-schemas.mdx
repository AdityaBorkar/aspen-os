...
Stores author (`ctx.actorId` required), body/title, `channel` (`general`/`hr`/`custom`), `audience` JSON (`type` + `ids`), `priority` (`normal`/`important`/`urgent`), `status` (`draft`/`scheduled`/`published`/`archived`), and `scheduled_for`.

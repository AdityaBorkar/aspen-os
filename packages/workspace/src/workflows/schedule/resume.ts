...
const ResumeInputSchema = object({ id: IdSchema });

export const resumeSchedule = Workflow.name("workspace.schedule.resume")
  ...
  .handler(async ({ id }, ctx) => {
    ...
    await ctx.step.run("register-cron", async () => {
      await registerScheduleDelivery({ audit: ctx.audit, db: ctx.db, pubsub: ctx.pubsub }, updated);
    });

    await ctx.audit.write({
      action: AUDIT_ACTION.RESUMED,
      crudAction: "update",
      entityId: id,

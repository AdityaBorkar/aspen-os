...
- **Fully implemented**: `platform`, `masters`, `organization`, `compliance`, `tasks`, `dms`, `management`, `hr`, `workspace` (modules), `constants` (shared enums). `drive` was **removed from the repo** and its file/folder/label/share/trash surface consolidated into `dms` (see Current State). `masters` owns the polymorphic master data (contacts/addresses/bank accounts/connections/notes/entities/payment methods + tenant-wide units of measure) extracted from `organization` (see Current State).

...
Client-side orchestrator class. Created via `Platform.create(config, modules)` w/ 3 units (auth, logs, rpc). No database, no tenancy. Has `run(fn)` method that sets client-side context (module-level variable, not `AsyncLocalStorage`) w/ `{ auth, logs, rpc }`, invokes `fn`. generic `Platform` name exists only on client; server exports `PlatformInstance` type only.
...
**GetUnit**:
Typed accessor to retrieve Unit by name after creation. Requires name — no zero-arg overload.
_Avoid_: Resolve, Get

**GetModule**:
Typed accessor to retrieve Module by name. Requires name — throws if not found. No zero-arg overload.
_Avoid_: Resolve, Get
...
**Session**:
Time-bounded authentication token tied to User. Has `id`, `token`, `userId`, `expiresAt`. Cascades delete from User. `AuthConfig` extends `BetterAuthOptions`, so session expiry configured via `AuthConfig.session.expiresIn` + forwarded into `betterAuth({ ...config, ... })`; better-auth handles expiry internally.
_Avoid_: Token, Login

**Account**:
Credential record linking User to authentication provider (email/password, OAuth, etc.). Stores `password`, `accessToken`, `refreshToken`, provider metadata. Not same as User.
_Avoid_: Credential, AuthMethod

**Role**:
Plain text field on User table. In Recruiter app, values `admin`, `bd`, `caller`, `qc`, `rm`, `sc`, `tl`. Not separate entity — no dedicated role table exists (HR's own RBAC is separate sub-domain).
...
**Archive**:
Soft-delete that moves file to new key + marks original as archived.
...
**Procedure**:
Named RPC handler w/ typed input/output. Built-in: `echo`, `health.check` (trivial ok-probe, not platform aggregate).
...
**TTL**:
Time-to-live on KV entry. Expired entries lazily evicted on read, not by background job.
...
> There is no `packages/organization` on disk. former organization module's surface now lives in **Masters** (`p.masters.orgBranches`: `create`, `get`, `list`, `tree`, `update` over `org_branch` table) and **Management** (`p.management.organizations`, control-plane read model). Organization profile fields (name, branding, logo) live in Masters settings (`org.*` keys). Do not import `@aspen-os/organization` — it does not exist.
...
**Payment Method**:
Mode of payment w/ `type` (`PAYMENT_METHOD_TYPE`: bank_account/card/upi/imps/cheque), `direction` (`inbound`/`outbound`/`both`), `status` (`active`/`inactive`/`archived`), type-specific inline detail fields (card: `cardBrand`/`cardLast4`/expiry; upi: `upiId`; bank-backed: `accountHolderName`/`accountNumber`/`bankName`), per-`(entityType, entityId, direction)` `isPrimary` flag. There is no `master_bank_account` table — bank details live inline on payment method. **Card data masked-only** — no PAN, no CVV, no token refs.
_Avoid_: Payment, Transaction, Ledger (this is method _configuration_, not payment execution)

**Label (Masters)**:
Scope-keyed tag (`name`, `color`, optional `(scopeType, scopeId)`; null pair = global) applied through `master_entity_label` join (unique per `(entityType, entityId, labelId)`). Surfaced as `p.masters.labels` (`create`, `apply`, `remove`, `list`, `listByLabel`); DMS subscribes to `masters.label_created/_updated/_removed` to mirror its cache.
...
**Verification Status**:
Lifecycle state of Compliance Document: `draft` → `submitted` → `under_review` → `verified`/`rejected` → `expired`/`overdue` → `renewed`/`archived`. Status derived from dates + renewal state by `StatusDerivation` service, not set directly.
_Avoid_: Document State, Approval Status

**Renewal Chain**:
Linked sequence of Compliance Documents where each new document archives previous via `renewedFrom` FK. Chain preserves renewal history for given obligation or entity.
...
**Event Bridge**:
Service that subscribes to external module events (`hr.employee_onboarded`, `hr.employee_separated`, `fleet.vehicle_registered`, `masters.org_branch_created`, `accounting.financial_year_started`, `masters.contact_created`) + auto-creates relevant Compliance Documents + Obligations based on event type. separate healthcare bridge handles `healthcare.operations_created`. There is no compliance-owned Reminder Engine — expiry nudges are calendar reminders.
_Avoid_: Event Listener, Integration Hub

### Tasks Domain

**Project**:
Container for tasks w/ unique `key` (e.g. `PROJ`), `name`, `status` (active/archived/paused), `leadId`, `taskCounter` for sequential task numbering, optional `defaultTaskTypeId`. Members added w/ roles (admin/member/viewer). Lives in control-plane schema (`task_project`); lead auto-added as admin on creation; cannot delete project that still has tasks.
_Avoid_: Board, Workspace

**Task**:
Unit of work within Project. Has `title`, `description`, `priority` (urgent/high/medium/low/none), `statusId`, `projectId`, `reporterId`, `assignees`, `labels`, `parentId` (max 3 levels nesting), `dueDate`, `estimatedHours`, `taskNumber` (display: `KEY-seq`). Supports archiving + soft-delete.
...
**Task Link**:
Typed relationship between two tasks: `blocks`, `blocked_by`, `related_to`, `duplicates`, `caused_by`, `split_from`. Creating link auto-creates its inverse. Cycle detection prevents circular dependencies.
_Avoid_: Task Relation, Dependency

**Saved View**:
Reusable filter/sort/group configuration w/ `name`, `type` (list/board/calendar/timeline), `filters` (jsonb), `sort` (jsonb), `groupBy`, `isShared`, `isDefault`. Owned by user, optionally scoped to project. (Cross-domain saved views live in Workspace as `Filter View`.)
...
**Time Entry**:
Logged time record on task w/ `duration` (minutes), `date`, `description`, `billable` flag, `userId`, `taskId`.
_Avoid_: Timesheet, Time Log

**Task Reminder**:
Time-bound follow-up on task. **Owned by `@aspen-os/calendar`** — task reminders are `calendar_reminder` rows w/ `targetType = task`, materialized by calendar task bridge from `task.due_date_changed` (three `due_date` rows per recipient: due − 1d, due − 1h, due), deleted on task delete, suppressed on completion/cancellation.
_Avoid_: Alert, Notification

**Watcher**:
User subscribed to updates on task. Watchers receive notifications when task updated, commented on, or status-changed.
_Avoid_: Subscriber, Follower

**Activity Log** (Tasks — exempt projection):
Append-only record of task actions: `task_created`, `task_updated`, `status_changed`, `assignee_added`, `assignee_removed`. Has `oldValue`, `newValue` (jsonb), `userId`, `taskId`. **Harmonized:** Platform `AuditLog` is store; per-module `ActivityFeed` is projection over `audit_log` (DMS `workflows/activity`, Compliance `workflows/audit`). Tasks `task_activity_log` is exempt (high-volume task timeline) — document exemption or migrate to projection.
...
**Attendee**:
Invitee on event — `email` + optional `name`/`attendeeId`/`attendeeType` (`user`/`contact`), `optional`, `status` (`invited`/`accepted`/`declined`/`tentative`). `add` publishes `calendar.attendee_invited`.
...
**Channel**:
Named sender endpoint (`from`: email address, WhatsApp number, SMS sender ID) plus credentials to send from it. `tenant` (BYOC — credentials in tenant kvStore via `credentialRef`) or `host` (references `comms_provider`; carries no credential material).
...
**Notification**:
Persisted intent + in-app inbox row. row **is** inbox — in-app delivery is zero extra work. Has `type`/`title`/`body`, `severity`, `recipientType`/`recipientId`, `to` snapshot, `channelTypes[]`, `status` (unread/read/dismissed), `sourceModule`/`sourceEntity`.
_Avoid_: Alert, Inbox Item

**Message**:
Delivery outbox row (`comms_message`) — one per outbound send. Status lifecycle `queued → sending → sent → delivered/failed`, w/ attempts/retries + provider receipts. Swept by `comms.message-sweeper` cron (`* * * * *`), which dispatches per-message in tenant context. Never `comms.deliver` topic.
_Avoid_: Event, Payload

**Recipient**:
`to`: internal user (resolved via Auth unit) or external contact (address carried in producer payload). Separate from Channel (`from`) by design — comms reads no other module's tables.
_Avoid_: Target, Destination

**Default Channel**:
At most one `isDefault` per `(type, entityType, entityId)`. Host defaults materialize lazily; default must be `active` and verified.
_Avoid_: Fallback Channel

**Preference**:
Per-user routing + consent row: `(userId, type, channelType)` opt-outs plus `(userId, null, channelType)` default row. `inapp` is routing-only pseudo channel type — never real channel.
...
**Triage**:
Staging stage for files uploaded without folder. Triaged file not searchable, normally listable, or shareable until classified. Only exit = `classify()` (→ `active`). Can be pinned via workspace module.
_Avoid_: Inbox, Draft Folder, Pending Queue

**Classify**:
Validation-enforced transition that assigns triaged File to Class, validates required fields, optionally applies class's file-naming schema, assigns `docNumber`, sets status `active`. One and only way out of Triage.
_Avoid_: File Into, Assign Class, Register

**Class**:
Admin-defined template w/ typed fields (some required) that File must satisfy to become active in that class. Optionally defines file-naming schema w/ field/date/sequence placeholders + per-class retention period. Archived (not hard-deleted) when superseded.
_Avoid_: Document Type, Category, Template

**Class Field**:
Typed column of Class (`text`/`number`/`date`/`select`/`multi-select`/`boolean`/`user`/`contact`/`url`/`email`/`phone`) w/ required/default/options/order. Field values stored as jsonb on File, optionally indexed for search.
_Avoid_: Column, Attribute, Metadata Key

**File Version**:
Stored revision of File. Storage keys version-bound (`dms/{tenant}/{fileId}/v{n}/{name}`), so `newVersion` writes fresh object + prune retains `maxVersions` (skipped under legal hold). Renames + moves metadata-only — never S3 move.
...
**Contact (DMS)**:
Org-wide address-book entry used as sharing handle for external parties; may be linked to internal AuthUnit user. Removal requires mandatory reason + revokes all shares granted to contact. Removal is driven by `masters.contact_removed` (contact-share bridge) — DMS owns no contact table.
_Avoid_: Sharee, External Recipient, Address Book Entry

**Share (DMS)**:
Permission grant (`viewer`/`editor`/`owner`) on File or Folder to grantee — Contact (token-based access, no login required), internal User, or Group. Revoking, or removing contact, invalidates access immediately. Folder grants inherit down folder tree.
_Avoid_: External Link, Access Grant

**Public Link (DMS)**:
Token-based shareable link (optional password, `view`/`edit` permission, optional `maxViews`/`expiresAt`) for File or Folder, managed by same `shares` group as grants.
_Avoid_: External Link, Share Link

**Legal Hold**:
Admin-placed flag (w/ mandatory reason) that blocks permanent deletion + auto-purge of File + stops version pruning. Released only by admin.
...
**Label (DMS)**:
Color-coded tag (`name`, `color`, global or owner-scoped) applied to Files + Folders through polymorphic `dms_entity_label` join (plus `dms_label_cache` mirror). File/folder can carry multiple labels; upload accepts `labelIds`. Label lifecycle events (`masters.label_created/_updated/_removed`) are consumed via label bridge.
...
> `@aspen-os/workspace` module. Term **Workspace** here = **personal-workspace surface** — drafts, filter views, dashboards, utilities — deliberately NOT Tenancy, NOT tasks Project/Board (both list "Workspace" as avoid term). See `.working-docs/domain-model/workspace.md`.

**Draft**:
Saved, unpublished piece of content — `title`, `body` (markdown/text), `notes`, `metadata` (opaque) — w/ optional approval lifecycle (`draft → submitted → approved → published`, `reject` → `reopened` to `draft`), soft-delete trash, duplicate, threaded comments (`workspace_draft_comment`). First-class persistable entity — NOT "draft" status value used by other modules (compliance documents, hr contracts).
_Avoid_: Draft Status, Staging Content

**Approval**:
Optional `submit → approve` gate on Draft; hosts without review step call `publish` directly from `draft` (approval not mandatory).
_Avoid_: Review, Sign-off (informal)

**Filter View**:
Cross-domain saved filter/sort/group configuration: `domain` (free-form `<module>:<entity>` text, validated by `DOMAIN_REGEX`), `conditions` (`{ field, operator, value }`), `sort` (`{ field, direction }`), `groupBy`, `isDefault` per `(ownerId, domain)`. Documented domains (`FILTER_VIEW_DOMAIN`): `workspace:draft`, `tasks:task`, `dms:file`, `compliance:document`, `hr:employee`, `notes:note` — any `module:entity` string passes validation; module never queries other modules' tables and there is no resolver registry in code. Datasource rule: non-embed widgets require `domain` + exactly one of inline `filter` or `viewId` soft-FK.
...
**Pricelist**:
Named price list bound to branches; `publish` freezes version, `bulkRevision` reprices, `resolvePrice` picks branch-applicable price.
_Avoid_: Price (a resolved number, not the list)

**Appointment**:
Booked visit — slot computation, queue tokens (`callNext`/`checkin`), video sessions, certificates, recalls. `reschedule` requires reason.
...
**Tenant ID**:
String identifier for tenant context of request. `"$global"` addresses control plane; any other ID addresses tenant (resolved from authenticated session, e.g. better-auth's `session.activeOrganizationId`, + passed to `platform.run(tenantId, fn)`). Stored in `AsyncLocalStorage` context. Used by stable DB wrapper to route queries, `PubSubUnit` to route messages, `StorageUnit`/`KvStoreUnit` to prefix keys.
...
**Stable DB Wrapper**:
JavaScript `Proxy` returned by `DatabaseUnit.db` (getter). Created once at init time, stored by workflows as `this.db`. When any property accessed (e.g. `this.db.select()`), wrapper reads per-request drizzle instance from `AsyncLocalStorage` + delegates to it. Transparent to workflows — no workflow code changes.
...
**Service Provider**:
First-class platform entity — implementation/integration partner that does physical-world onboarding work for Tenant (site setup, install, training). Each Tenant has at most one active Service Provider at time (1:1 active assignment); SP may serve many Tenants. SP's staged work happens during Tenant's `onboarding` stage. Lives in own table in control-plane DB; not Tenant subtype, not Masters `Connection`.
...
User w/ `user.role = 'sp_user'` + `service_provider_user` join row pointing to their Service Provider. Zero tenant `member` rows. Field staff working for SP — can view assigned Tenants, update onboarding status, upload install/training artifacts. Scope = SP they belong to, not tenant.
...
**Report**:
Read-only view produced by Management Plane over control-plane DB. Four categories: (1) tenant usage metrics (users, modules, storage, API calls per tenant), (2) provisioning & lifecycle reports (tenants by lifecycle stage, assigned SP, time-in-onboarding, churn reasons), (3) audit & activity reports (who created/suspended/churned tenant, SP assignments, role changes, platform admin actions), (4) SP performance reports (tenants per SP, avg onboarding duration, completion rates). All reports control-plane queries; never cross into per-tenant DBs.

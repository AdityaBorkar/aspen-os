...
- **Separate management-portal app** (own Platform + own AuthUnit for platform admins; tenant apps stay single-tenant): cleaner per-app but leaves the SaaS without a unified multi-tenant data plane and forces cross-app coordination for every tenant operation.

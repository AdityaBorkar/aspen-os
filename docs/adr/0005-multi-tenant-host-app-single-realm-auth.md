...
> three tenancy modes as a config-time choice (single, shared-RLS, isolated-DB). This ADR's
> description of a multi-tenant host app with single-realm auth remains valid for the
> `isolated-db` mode specifically. The `tenant_id` column requirement described below is revised
> — see ADR-0007 for the updated schema approach (always present, not added per-query).

The Tenant Platform is implemented as a module registered into a new multi-tenant host app that replaces the single-tenant Recruiter app. One `Framework.create()` instance, one `AuthUnit`, one better-auth instance, one set of auth tables — serving both platform admins and tenant end-users in a single `user` table.
...
- Every existing module table (`organization`, `branch`, `task`, `drive_file`, all 44 HR tables, etc.) gains a `tenant_id` column. Per ADR-0007, this column is always present with `DEFAULT 'default'` — not added per-query. In `isolated-db` mode, it's redundant per database. In `shared-rls` mode, RLS policies filter by it.
...
The Tenant Platform module's own tables (`tenant`, `service_provider`, etc.) live in the default schema of the control-plane database — no separate `management` schema (consistent with how other modules use the default schema). The SOW's "schema name 'management'" line is revised away.

...
> one of three selectable tenancy modes (`isolated-db`), not the only option. The framework
> supports `single`, `shared-rls`, and `isolated-db` modes as a config-time choice. This ADR
> describes the `isolated-db` mode specifically. The rejection of app-level `tenant_id` filtering
> and RLS below was the decision for the tenant-platform host app; ADR-0007 makes RLS a
...
- Tenant databases must be provisioned (created, schema pushed, optionally seeded) — this is a Tenant Platform module responsibility, likely via a workflow that issues `CREATE DATABASE`, runs `pushSchema()` against the new DB, and records the connection params in the control-plane `tenant` table.
...
- Platform admins do NOT touch the data-plane directly. The Tenant Platform module operates only on the control-plane DB. If a platform admin needs to inspect a tenant's data, they use better-auth's admin-impersonation (`signInAsUser`) to act as a tenant admin. No "act as tenant" mechanism is needed in the module itself.

...
Move config functionality to config

start working on healthcare

Split the healthcare module into:

- Separate all laboratory and radiology related functionality into separate "diagnostics" module.
- Split all pharmacy related functionality into separate "pharmacy" module.
- Separate all Medical Records (allopathy, dental, ayush, rehab, psych) related functionality into separate "emr" module.
- Separate all nursing and residents related functionality into separate "inpatient" module.

`healthcare-clinic`
at the same time, work on HR implementation

CREATE A PHARMACY ONLY APP NAMED `healthcare-pharmacy`
at the same time, work on inventory, pricelists, services, goods, billing/accounting/sales

`healthcare-hospital`
at the same time, work on HEALTHCARE implementation

Add Feature List to the docs

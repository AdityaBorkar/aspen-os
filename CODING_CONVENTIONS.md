...
Exhaustive code-facts reference. `AGENTS.md` is lean pointer; detail lives here. Every rule below matches code — grep-verified.

Task routing: write new module → `.agents/skills/write-module/SKILL.md` + §3 Module shape; add/change table or column → §3 Database; add/change validation → §3 Validation; add/change event or workflow → §3 Events/Workflows; add/change ACL or auth → §3 Auth; understand platform lifecycle or units → §3 Module shape; name anything → §3 Naming summary; build/typecheck/verify → §3 Build/ops.
...
Workspace layout: `packages/*`, `examples/*`, `docs`. `examples/*` contains only `examples/recruiter/seaweedfs-s3.json` (SeaweedFS/S3 config stub, no `package.json`); there is no host/example application. `docs` is Fumadocs site; domain source of truth is `.working-docs/` (drafts in `.draft/`).

Build-step packages (have `build` script `bun run ../../scripts/build.ts` → `.output/`): `platform`, `masters`, `notes`, `calendar`, `management`, `comms`, `dms`, `workspace`, `healthcare`, `constants`. All except `constants` rewrite `exports`/`bin` to `.output/` on build; `constants` keeps `exports` at `./src/index.ts` and only emits declarations to `.output/`. Raw-src packages (no build, export raw `src`): `announcement`, `compliance`, `tasks`, `hr-core`, `hr-attendance`, `hr-leave`. Stub packages (`crm`, `fleet`, `inventory`, `reports`): `package.json` holds only `name`; `src/index.ts` is empty; `docs/` holds stub `index.mdx`/`meta.json`/`overview.mdx`. No `organization` package exists on disk.
...
- `nx.json`: `parallel:20`, `targetDefaults.build: { dependsOn:["^build"], outputs:["{projectRoot}/.output"], cache:true }`, `check:lint: { dependsOn:["^check:lint"], cache:true }`, `check:types: { dependsOn:["^check:types"], cache:true }` — `check:types` depends only on `^check:types`, never on `^build`.
...
Break any rule below and `check:types` or `check:lint` fails. cited tool is enforcer; no manual prohibition phrasing needed here.
...
- Import sorting — `oxfmt` sorts imports by groups `url` → `builtin/protocol/subpath` → `external` → `internal/parent/sibling/index/style` → `unknown`; violation is auto-fixed but `check:lint` mutates file.
...
Every item below is imperative or explicit prohibition. Pair each prohibition with its replacement in same sentence.

### Naming

- Use `kebab-case` for all filenames, never `camelCase` or `snake_case` for files; oxlint `unicorn/filename-case` enforces it but convention is hand-documented for intent.
...
- Use `snake_case` for database tables and columns in both Postgres and TypeScript, never `camelCase` in DB code; write `owner_id: text().notNull()` never `ownerId: text("owner_id")`; TS object key is column name — never pass explicit name string inside column builder.
- Use `domain.event_name` for event topics, never `domainEventName` or `domain:event_name`; write `"dms.file_uploaded"` never `"dms.fileUploaded"` or `"dms:file_uploaded"`.
...
- Keep Naming summary table in sync when any naming convention changes.
...
- Never pass explicit name to `uuidv7()` — use `uuidv7().primaryKey()`, never `uuidv7("id")`; column helper infers name from object key.
- Use exported `uuidv7` from `@aspen-os/platform/server` (backed by `generateUuidv7()` using `crypto.getRandomValues` and `timestamp` byte layout), never `sql\`uuidv7()\``or raw`crypto.randomUUID()`for domain IDs;`generateUuidv7()`stays available for non-column UUID strings, but workflow engine uses`crypto.randomUUID()`for`run_id`/`step_id`.
...
- Use `.notNull().defaultNow()` for `created_at` in domain/core tables, never bare `timestamp()`; add `$onUpdate(() => new Date())` only where `updated_at` must auto-bump, never omit trigger when update tracking is required.
...
- Use `idx_<table>_<column>` for ordinary indexes and `idx_<table>_search` for GIN `to_tsvector` full-text indexes, never `index_<table>` or unnamed indexes; declare indexes in table's third argument as `index()` array or object map; DMS adds GIN indexes over `to_tsvector('simple', name) || to_tsvector(...)` expressions.
...
- Never add explicit Drizzle foreign keys on domain tables — store related IDs as plain `text` without `.references()`; exception — generated better-auth tables use explicit `references(() => user.id, { onDelete: "cascade" })` after re-applying snake_case codemod post-generation.
...
- Never create migration files — use `pushSchema()` (`DatabaseUnit.$prepareInfra` / `prepareWithModules` / `pushSchemasToTenant`); `pushSchema` is currently commented to no-op but contract remains — do not add `drizzle-kit` migration scripts.
- Never create `Result<T, E>` or `PaginatedResult` types — use thrown errors and direct return values.
- Map at DB boundary explicitly: Valibot schemas, event payloads, `AuditEntry`, and context (`ctx.actorId`, `ctx.tenantId`) stay `camelCase`, but inserts/updates use `snake_case` keys with camel values (`owner_id: input.ownerId`); row reads use `snake_case` (`created.owner_id`), event publishes map back (`ownerId`).

### Module shape

- Always implement `Module` interface, never bare class without `implements Module`; define `readonly $name`, `readonly $dependencies`, `$initialize`, `$prepareInfra`, `$prepareRuntime`, `$cleanup`, and optionally `$prepareTenant` for isolated-only modules.
- Use `static create(config)` factory, never `new Module()` directly; store `readonly $config` and apply defaults in constructor (e.g. `DEFAULT_CONFIG` spread).
- Always declare `readonly $dependencies: readonly string[]` (type `[]` when empty), never omit field; validation happens in `BasePlatform.createCore()` which throws if listed dependency is missing; units (`db`, `pubsub`, etc.) must never be listed in `$dependencies` — they arrive via `$initialize(units)`.
- Use `readonly $consumes: readonly string[]` for optional peer event topics, never validate `$consumes` at creation time; use it as introspection-only (`compliance` consumes `hr.*`, `fleet.*`, `masters.*`; `dms` consumes `masters.contact_removed`; `calendar` consumes `task.*`); missing producer silently no-ops.
- Follow lifecycle order `Platform.create(config, modules)` → `$prepareInfra()` → `run(...)` → `$cleanup()`, never call `$prepareRuntime` or `run` before `$prepareInfra`; creation validates `$dependencies`, calls `mod.$initialize(units)`, returns Proxy resolving unit keys before module `$name`; `$prepareInfra` merges schemas/ACL, pushes schemas, applies ACL, then runs `$prepareRuntime` inside `runInContext`.
- Always call `getContext()` inside `Platform.run()` or `pubsub.wrapHandler` context, never outside; `getContext()` throws outside `AsyncLocalStorage` scope.
- Use ` $initialize(units)` to capture unit refs, never store full `units` map untyped; stateless modules (e.g. `tasks`, `notes`, `healthcare`) keep empty `$initialize`/`$prepareRuntime`/`$cleanup` and expose `readonly` workflow groups; runtime-wired modules (`compliance`, `calendar`, `dms`, `workspace`, `comms`, `hr-*`, `management`, `masters`) keep `#private` unit refs, register schedules/subscriptions in `async $prepareRuntime()` and unregister/null them in `$cleanup()`.
...
- Use `Record<string, Unit>` with `isUnit(unit, "db")` type guards when injection shape varies, never unguarded casts; typed `{ db, auth, pubsub }` is allowed when module's `$initialize` narrows to required units.
...
- Follow file structure exactly, never deviate without updating this document:
  ```
  packages/<module>/
...
- Use uniform non-overloaded `run(tenantId, fn)` signatures, never overloaded `run()`; `SingleTenantPlatform.run(tenantId, fn)`, `SharedTenantPlatform.run(tenantId, fn)`, `IsolatedTenantPlatform.run(tenantId, fn)` are distinct; `"$global"` routes to control-plane DB; shared mode uses RLS transaction (`SELECT set_config('app.tenant_id', ...)` + `SET LOCAL ROLE tenant_role`), isolated mode uses `TenantResolver` DB-per-tenant, `isGlobalTenantId("$global")` routes to control-plane DB; do not add overloaded `run()`.
...
- Use `export type DomainEventMap = EntityEventMap & OtherEventMap` composed by intersection, never single flat map; each `EventMap` maps `[ENTITY_EVENTS.X]` to its interface.
- Pass `events = { ENTITY_EVENTS, ... }` through `ModuleInfra.events` as type-level contract only; platform has no runtime event side effects, never rely on `ModuleInfra.events` for publishing.
...
- Use `defineAcl({ resource: ["create","read","update","delete", ...] })` in flat `src/auth.ts`, never `utils/acl.ts` or inline ACL; `defineAcl` is identity fn with `const` generic for literal inference (`AclDeclaration = Record<string, readonly string[]>`); platform merges all module ACLs concatenating unique actions per resource and calls `AuthUnit.applyModuleAcl(mergedAcl)` which recreates better-auth with `admin({ ac: createAccessControl(acl) })`, never `admin({})` alone at runtime.
...
- Use `text("role")` on `user` for roles, never separate role table; better-auth plugins are `admin`, `username`, `organization`, `phoneNumber`, `emailOTP`, `apiKey`, `twoFactor`, `passkey` (with commented `LastLoginMethod`); client adds `LastLoginMethodClient` + `CaptchaClient`.
- Never hand-edit `packages/platform/src/server/db/schema/auth.gen.ts` — regenerate via `bun run gen:auth-schema` (`bunx auth generate --config ./src/server/auth/~config.ts --output ./src/server/db/schema/auth.gen.ts`) and re-apply snake_case codemod; that file is 10-table generated better-auth schema (`user`, `session`, `account`, `verification`, `organization`, `member`, `invitation`, `apikey`, `twoFactor`, `passkey`).
- Use `remove` in auth REST API, never `delete`; surface is `user.{create,get,remove,role.assign,role.unassign,update}`, `session.{create,invalidate,validate}`, `role.{list,remove}`, `otp.get`.

### PubSub

- Never call `pubsub.$prepareInfra()` expecting eager start — single control-plane `pg-boss` is lazily started on first `publish`/`subscribe`/`schedule`, not during `$prepareInfra()`; `publish` auto-creates missing queue and retries, so fire-and-forget works without prior consumer — jobs queue durably until subscriber appears, never assume unsubscribed topic throws.
- Always ensure every produced topic has subscriber; `PubSubUnit.getUnsubscribedProducedTopics()` tracks produced topics lacking `subscriptions.has(topic)`; pg-boss silently drops these, so liveness (RPC `health.check` + unsubscribed-topic check) flags producer/consumer wiring bug — `publish()` warns but does not throw on no-id result.
...
- Use `bun run check:lint` (mutating `oxlint --fix . ; oxfmt .`) and `bun run check:types` (`tsc -b` composite) for verification, never bare `tsc` without `-b`; run focused checks via `cd packages/<name> && bun run check:lint` or `bun run check:types` (only in packages with those scripts — `crm`, `fleet`, `inventory`, `reports` are scriptless stubs); `nx.json` makes `check:types` depend only on `^check:types`, never on `^build`; prefer `nx run-many -t <target>` over raw per-package loops for cross-package verification.
- Use `bunx fumadocs-mdx` when `docs/.source/` is missing, never assume install generated it (`ignore-scripts=true` blocks it); docs commands run from `docs` and always start with `gen:ref`: `bun run dev` (`gen:ref` + Vite on port 3005), `check:types` (`gen:ref` + `fumadocs-mdx` + `tsc --noEmit`), `build` (`gen:ref` + `gen:cf-types` + Vite), `deploy` (`wrangler deploy`).
...
- Never provide external PostgreSQL via checked-in compose — no compose file exists; tests and runs need externally provided Postgres; S3 via SeaweedFS stub `examples/recruiter/seaweedfs-s3.json` is only infra hint.
- Never commit files violating `.oxlintrc.json` or `tsconfig.json` rules — `check:lint` and `check:types` are gates; maintained custom test suite is `cd tools/oxlint/anti-slop && bun test`; no package test scripts or CI workflows exist.
...
- Never use `sql\`uuidv7()\``or`crypto.randomUUID()`for domain IDs — use exported`uuidv7` helper.
- Never pass `uuidv7("id")` with explicit name — use `uuidv7().primaryKey()` and let key infer column name.
...
- Never use `delete` in auth REST API — use `remove`.
- Never recreate `drive` package or parallel file/tag/share/trash model — `@aspen-os/dms` is single document/file surface (`dms_file`, `dms_folder`, `dms_share`, `dms_label`, `dms_share`, `dms_setting`, `dms_legal_hold`, `dms_access_log`, `dms_file_version`, `dms_class`, `dms_class_field`, `dms_entity_label`, `dms_public_link`).
...
- When rule starts being enforced by `oxlint`/`oxfmt`/`tsc`, move it from §3 to §2; when check stops enforcing, move it down to §3.
- Keep `AGENTS.md` as pointer and this file as exhaustive home; never create third copy of convention.
- Keep Naming summary and this section in sync when any naming rule changes.

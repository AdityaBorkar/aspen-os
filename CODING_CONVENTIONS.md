...
- Use `bun run check:lint` (mutating `oxlint --fix . ; oxfmt .`) and `bun run check:types` (`tsc -b` composite) for verification, never bare `tsc` without `-b`; run focused checks via `cd packages/<name> && bun run check:lint` or `bun run check:types`; `nx.json` makes `check:types` depend only on `^check:types`, never on `^build`; prefer `nx run-many -t <target>` over raw per-package loops for cross-package verification.

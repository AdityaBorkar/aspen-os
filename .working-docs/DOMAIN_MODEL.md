...
- Always `id: uuidv7().primaryKey()` — never native UUID columns, never an explicit name (`uuidv7("id")` is forbidden). `uuidv7` is the Drizzle column type exported from `@aspen-os/platform/server` (SQL `text`), and it generates the UUIDv7 at insert time in JS.
- **Exception 1**: better-auth tables (`user`, `session`, `account`, `verification`, `organization`, `member`, `invitation`, `apikey`, `twoFactor`, `passkey`) use `text().primaryKey()` without a default.
- **Exception 2**: `management.tenant.id` uses `text().primaryKey()` without a default; onboarding supplies the ID.
...
- Table names `snake_case` (DMS carries a `dms_` prefix); column names `snake_case` in **both** Postgres and TypeScript — the TS object key **is** the column name, never repeated inside the datatype params (`owner_id: text().notNull()`, never `ownerId: text("owner_id")`). Columns sorted alphabetically by key.
- API boundary stays `camelCase` (Valibot input schemas, event payloads, platform context) with explicit mapping at the DB boundary.
...
1. **All IDs are text** — app-generated via the `uuidv7()` column type (bakes in the insert-time JS `generateUuidv7()` default), except better-auth tables and `management.tenant.id`.
...
5. **No DB-level foreign keys in domain modules** — compliance, tasks, organization, masters, management, healthcare, and hr all use soft FKs.

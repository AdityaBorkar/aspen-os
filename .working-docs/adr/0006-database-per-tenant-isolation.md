...
- Existing module tables (organization, branch, task, dms_file, employee — 14 HR setup/access tables live in the control plane, the other 36 including `employee` are tenant-side, etc.) do NOT gain a `tenant_id` column — isolation is by database, not by row. They remain structurally single-tenant; the tenant is implied by which database the query runs against.
